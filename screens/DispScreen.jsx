...
export default function DispScreen({ navigation }) {
                ...
                .then((re) => {
                    ...
                    var ints = re.data.filter((viaje) => {
                        if (viaje.interno.lineaId === lineas[indexLinea].id
                            && new Date(viaje.ruta.vuelta.destination.time) >= new Date()) {
                            return viaje
                        }
                    })
                    setViajes(ints)
    ...
    const mostrarRuta = (viaje) => {
        ...
        if (isSwitchOn) {
            ...
            var cordenadas = viaje.ruta.ida.route.map((e) => {
                return { latitude: e.lat, longitude: e.lng }
            })
        } else {
            setRuta(viaje.ruta.vuelta)
            var cordenadas = viaje.ruta.vuelta.route.map((e) => {
                return { latitude: e.lat, longitude: e.lng }
            })
        }
        setCord(cordenadas)
            ...
            {!viaje ? <View style={{
                ...
                {viajes ? viajes.map((dat) => (
                    <List.Item
                        ...
                        description={
                            new Date(dat.ruta.ida.origin.time).getHours().toString().padStart(2, '0') + ":" + new Date(dat.ruta.ida.origin.time).getMinutes().toString().padStart(2, '0')
                            + " - "
